...
public class TownyEconomyHandler {
	...
	private static Object getEconomyAccount(String accountName) {
		switch (Type) {
			case REGISTER:
				if (!Methods.getMethod().hasAccount(accountName)) Methods.getMethod().createAccount(accountName);
		...
		try {
			switch (Type) {
				...
				case VAULT: // Attempt to zero the account as Vault provides no delete method.
					if (!vaultEconomy.hasAccount(accountName)) vaultEconomy.createPlayerAccount(accountName);
	...
	public static double getBalance(String accountName, World world) {
		switch (Type) {
			case REGISTER:
				...
				if (registerAccount != null) return registerAccount.balance();
				break;
			case VAULT:
				if (!vaultEconomy.hasAccount(accountName)) vaultEconomy.createPlayerAccount(accountName);
	...
	public static boolean hasEnough(String accountName, Double amount, World world) {
		if (getBalance(accountName, world) >= amount) return true;
		return false;
	}
